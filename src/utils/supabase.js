...
export async function getAdminConfig(fallback = null) {
  ...
  return {
    ...
    mineMinBet:       Math.max(0.001, Number(data.mine_min_bet ?? 1)),
...
export async function saveAdminConfig(cfg) {
  const row = {
      ...
      mine_min_bet:     Math.max(0.001, Number(cfg.mineMinBet ?? 1)),
...
function dbTxToApp(t) {
  ...
  return {
    ...
    withdrawalAttemptId: t.withdrawal_attempt_id || '',
    withdrawalSentAt: t.withdrawal_sent_at || '',
    withdrawalConfirmedAt: t.withdrawal_confirmed_at || '',
