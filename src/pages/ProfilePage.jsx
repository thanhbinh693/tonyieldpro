...
export default function ProfilePage({ user, referral, referralDetails = [], config, showToast, setIsAdmin, isAdmin, walletConnected, disconnectWallet, connectWallet }) {
  ...
  const teamDetails = Array.isArray(referralDetails) ? referralDetails : []
  const teamStats = Array.isArray(referralDetails)
    ? {
        friends: teamDetails.length,
        commission: teamDetails.reduce((sum, item) => sum + (Number(item.referralIncome) || 0), 0),
        depositVolume: teamDetails.reduce((sum, item) => sum + (Number(item.totalDeposit) || 0), 0),
      }
    : {
        friends: Number(referral?.friends) || 0,
        commission: Number(referral?.commission) || 0,
        depositVolume: Number(referral?.depositVolume) || 0,
      }
        ...
        <div className="ref-full card">
          <div className="rf-header">
            <div>
              ...
              <div className="rf-sub">Earn {formatTon(referralRewardTon)} when an invited user unlocks withdrawals.</div>
            </div>
            <div className="rf-badge">{referralRewardTon.toFixed(3)}</div>
          </div>
          ...
          <div className="rf-stats">
            <div className="rfs-item">
              <div className="rfs-val">{teamStats.friends}</div>
              <div className="rfs-label">Referred Users</div>
            </div>
            <div className="rfs-item">
              <div className="rfs-val">{formatTon(teamStats.commission)}</div>
              <div className="rfs-label">Referral Income</div>
            </div>
            <div className="rfs-item">
              <div className="rfs-val">{formatTon(teamStats.depositVolume)}</div>
        ...
        <div className="ref-detail card">
          <div className="rd-header">
            <div>
              ...
              <div className="rd-sub">Valid members unlock withdrawals before referral rewards are credited.</div>
            </div>
            <div className="rd-count">{teamDetails.length}</div>
          </div>
  ...
  return (
    <div className="page page-enter">
      ...
      <AvatarCard
        ...
        referredUsers={teamStats.friends}
      />
